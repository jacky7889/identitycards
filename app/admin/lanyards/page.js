...
export default function LanyardsPage() {
  ...
  // Fetch all lanyards
  const fetchLanyards = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${READ_ADMIN_URL}/lanyards.php`);
      const data = await res.json();
      setLanyards(Array.isArray(data) ? data : []);
      // console.log(data);
    } catch (err) {
      console.error("Failed to fetch lanyards:", err);
      setLanyards([]);
    } finally {
      setLoading(false);
    }
  ...
  // Filter lanyards based on search query
  const filteredLanyards = useMemo(() => {
    if (!query) return lanyards;
    const q = query.trim().toLowerCase();
    return lanyards.filter((lanyard) => {
      return (
        String(lanyard.id).toLowerCase().includes(q) ||
        String(lanyard.lanyard_name || "").toLowerCase().includes(q) ||
        String(lanyard.lanyard_detail || "").toLowerCase().includes(q) ||
        String(lanyard.price || "").toLowerCase().includes(q) ||
        String(lanyard.color || "").toLowerCase().includes(q) ||
        String(lanyard.feature || "").toLowerCase().includes(q) ||
        String(lanyard.weight || "").toLowerCase().includes(q) ||
        String(lanyard.length || "").toLowerCase().includes(q)
      );
    });
  }, [lanyards, query]);

  // Sort lanyards
  const sortedLanyards = useMemo(() => {
    const arr = [...filteredLanyards];
    arr.sort((a, b) => {
    ...
    return arr;
  }, [filteredLanyards, sortBy, sortDir]);

  // Pagination
  const total = sortedLanyards.length;
  const totalPages = Math.max(1, Math.ceil(total / pageSize));
  const currentPageLanyards = useMemo(() => {
    const start = (page - 1) * pageSize;
    return sortedLanyards.slice(start, start + pageSize);
  }, [sortedLanyards, page, pageSize]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${CREATE_ADMIN_URL}/lanyard.php`, {
        method: "POST",
        body: formData,
      });
      ...
      console.error("Error adding lanyard:", error);
      alert("Failed to add lanyard.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!confirm("Delete this lanyard?")) return;

    try {
      const res = await fetch(`${DELETE_ADMIN_URL}/lanyard.php?id=${id}`);
      const data = await res.json();
      alert(data.message);
      if (data.success) fetchLanyards();
    } catch (err) {
      console.error(err);
      alert("Failed to delete lanyard.");
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 className="text-2xl font-bold">Lanyards</h1>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          >
            {showForm ? "Hide Form" : "Add New Lanyard"}
          </button>
        </div>
      </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <input
              ...
              value={form.lanyard_name}
              onChange={(e) => setForm({ ...form, lanyard_name: e.target.value })}
              required
            />
            <input
              type="text"
          ...
          <textarea
            placeholder="Lanyard Details"
            className="border p-2 w-full rounded"
            value={form.lanyard_detail}
            onChange={(e) => setForm({ ...form, lanyard_detail: e.target.value })}
            rows={3}
          />
      ...
      <div className="bg-white rounded shadow overflow-x-auto">
        <table className="min-w-full divide-y">
          ...
          <tbody>
            ...
            ) : currentPageLanyards.length === 0 ? (
              <tr>
                <td colSpan={columns.length + 1} className="px-4 py-6 text-center text-gray-500">
                  {lanyards.length === 0 ? "No lanyards found." : "No matching lanyards found."}
                </td>
              </tr>
            ) : (
              currentPageLanyards.map((row) => {
                ...
                return (
                  <tr key={row.id} className="border-t hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 font-medium">{row.lanyard_name || "N/A"}</td>
                    <td className="px-4 py-3 max-w-xs truncate" title={row.lanyard_detail}>
                      {row.lanyard_detail || "N/A"}
                    </td>
                    ...
                    <td className="px-4 py-3">{row.length || "N/A"}</td>
                    
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <button
                          title="Edit"
                          onClick={() => router.push(`/admin/lanyards/edit/${row.id}`)}
