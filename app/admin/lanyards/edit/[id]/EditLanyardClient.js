...
export default function EditLanyardClient({ id }) {
  ...
  // Fetch lanyard details
  useEffect(() => {
    const fetchLanyard = async () => {
      try {
        const res = await fetch(`${READ_ADMIN_URL}/lanyards.php?id=${id}`);
        if (!res.ok) {
        ...
        if (Array.isArray(data)) {
          lanyard = data.find((item) => String(item.id) === String(id));
        } else {
          lanyard = data;
        }
        ...
        if (lanyard) {
          setForm({
            ...
            lanyard_detail: lanyard.lanyard_detail || "",
            price: lanyard.price || "",
            color: lanyard.color || "",
            feature: lanyard.feature || "",
            weight: lanyard.weight || "",
            length: lanyard.length || "",
          });

          if (Array.isArray(lanyard.images)) {
            setImages(lanyard.images); // store only filenames
          }
        } else {
          alert("Lanyard not found");
          router.push("/admin/lanyards");
        }
      } catch (err) {
        console.error("Error fetching lanyard:", err);
        alert("Failed to load lanyard details");
      } finally {
        setLoading(false);
      }
    };
  ...
  // Submit updated lanyard
  const handleSubmit = async (e) => {
    e.preventDefault();
    
    // Validate required fields
    if (!form.lanyard_name?.trim()) {
      alert("Lanyard name is required");
      return;
    }
    ...
    try {
      ...
      formData.append("lanyard_name", form.lanyard_name.trim());
      formData.append("lanyard_detail", form.lanyard_detail?.trim() || "");
      formData.append("price", form.price || "");
      formData.append("color", form.color?.trim() || "");
      formData.append("feature", form.feature?.trim() || "");
      formData.append("weight", form.weight || "");
      formData.append("length", form.length?.trim() || "");
      ...
      if (result.success) {
        alert("✅ Lanyard updated successfully!");
        router.push("/admin/lanyards");
      } else {
        alert("❌ Update failed: " + (result.message || "Unknown error"));
      }
    } catch (error) {
      console.error("Error updating lanyard:", error);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading lanyard details...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-3xl mx-auto p-6 bg-white shadow rounded">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-4">
          <button
            type="button"
            onClick={() => router.push("/admin/lanyards")}
          ...
          >
            {isSaving ? "Updating..." : "Update Lanyard"}
          </button>
        </div>
      </form>
    </div>
